...
int main(int argc, const char * argv[]) {
    ...
    printf("The Server AND is up and running using UDP on port 22251.\n");
    ...
    for (int i = 0; i < count; i++) {
    	result[i] = doAnd(s1buf[i], s2buf[i]);
    	cout << s1buf[i] + " and " + s2buf[i] + " = " + result[i]<< endl;
    	//result[i] = "11011";
    }
    cout <<"The Server AND has successfully received "<< count;
    cout <<" lines from the edge server and finished all AND computations." << endl;
    memset(s1buf, 0, sizeof(s1buf));
    memset(s2buf, 0, sizeof(s2buf));
